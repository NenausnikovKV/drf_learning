...
class DjangoStyleViewFunctionTest(TestCase):
    ...
    def test_json_response(self):
        ...
        """
        address = shortcuts.reverse("function_view:json_response")
    ...
    def test_json_snippet_list(self):
        ...
        address = shortcuts.reverse("function_view:django_style_snippet_list")
        response = self.client.get(address)
        content_reader = io.BytesIO(response.content)
        data = JSONParser().parse(content_reader)
    ...
    def test_json_snippet_detail(self):
        ...
        address = shortcuts.reverse("function_view:django_style_snippet_detail", kwargs={"pk": 1})
        response = self.client.get(address)
        content_reader = io.BytesIO(response.content)
        data = JSONParser().parse(content_reader)
